...
namespace Xamarin.Robotics.Micro.Motors
{
    public class HBridgeMotor : BlockBase, IDCMotor
    {
        ...
        {
            var leftPwm = new Xamarin.Robotics.Micro.Devices.PwmOutputPin (enable);
            var leftMotor = new HBridgeMotor (leftPwm);
            leftMotor.A1Output.ConnectTo (new Xamarin.Robotics.Micro.Devices.DigitalOutputPin (a1).Input);
            leftMotor.A2Output.ConnectTo (new Xamarin.Robotics.Micro.Devices.DigitalOutputPin (a2).Input);
            return leftMotor;
        }
    }
}
