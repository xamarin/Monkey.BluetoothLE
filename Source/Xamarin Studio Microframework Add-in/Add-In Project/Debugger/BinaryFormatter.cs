...
namespace Microsoft.SPOT.Debugger
{
	...
	public enum SerializationFlags
	{
		...
		// Value uses range compression (max 2^30 values).
		Optional = 0x00000004,
		// If the value cannot be deserialized, skip it.
		...
		public byte[] ToArray()
		{
			...
			lock(m_lock)
			{
				for(int pass = 0; pass < 2; pass++)
				{
					...
					{
						if(pass == 1)
							Array.Copy(ptr.m_data, 0, res, tot, ptr.m_length);
		...
		public void WriteBits(uint val, int bits)
		{
			...
			lock(m_lock)
			{
				while(bits > 0)
				{
					...
					pos -= insert;
					m_current.m_data[m_pos] |= (byte)(((val >> pos) & mask) << (m_avail - insert));
					bits -= insert;
					m_avail -= insert;
				}
		...
		public uint ReadBits(int bits)
		{
			...
			lock(m_lock)
			{
				while(bits > 0)
				{
					...
					pos -= insert;
					val |= (((uint)m_current.m_data[m_pos] >> shift) & mask) << pos;
					bits -= insert;
					m_avail -= insert;
				}
			}
	...
	public class BinaryFormatter
	{
		...
		internal const uint TE_L1_Duplicate = 0x00000001;
		// N bits for the duplicate id.
		internal const uint TE_L1_Reference = 0x00000002;
		// 32 bits for the type.
		internal const uint TE_L1_Other = 0x00000003;
		internal const int TE_L2 = 2;
		internal const uint TE_L2_Primitive = 0x00000000;
		// 4 bits for the type.
		...
		internal const uint TE_L3_Type = 0x00000000;
		// 32 bits for the type.
		internal const uint TE_L3_Method = 0x00000001;
		// 32 bits for the type, N bits for the signature??
		internal const uint TE_L3_Field = 0x00000002;
		// 32 bits for the type, N bits for the signature??
		...
		struct TypeDescriptorBasic
		{
			...
			internal void SetType(Type t)
			{
				...
				//
				if(t == typeof(System.RuntimeFieldHandle))
					goto Failure;
				if(t == typeof(System.RuntimeMethodHandle))
					goto Failure;

				if(t == typeof(System.Boolean))
					m_et = ElementType.PELEMENT_TYPE_BOOLEAN;
				else if(t == typeof(System.Char))
					m_et = ElementType.PELEMENT_TYPE_CHAR;
				else if(t == typeof(System.SByte))
					m_et = ElementType.PELEMENT_TYPE_I1;
				else if(t == typeof(System.Byte))
					m_et = ElementType.PELEMENT_TYPE_U1;
				else if(t == typeof(System.Int16))
					m_et = ElementType.PELEMENT_TYPE_I2;
				else if(t == typeof(System.UInt16))
					m_et = ElementType.PELEMENT_TYPE_U2;
				else if(t == typeof(System.Int32))
					m_et = ElementType.PELEMENT_TYPE_I4;
				else if(t == typeof(System.UInt32))
					m_et = ElementType.PELEMENT_TYPE_U4;
				else if(t == typeof(System.Int64))
					m_et = ElementType.PELEMENT_TYPE_I8;
				else if(t == typeof(System.UInt64))
					m_et = ElementType.PELEMENT_TYPE_U8;
				else if(t == typeof(System.Single))
					m_et = ElementType.PELEMENT_TYPE_R4;
				else if(t == typeof(System.Double))
					m_et = ElementType.PELEMENT_TYPE_R8;
				else if(t == typeof(System.String))
					m_et = ElementType.PELEMENT_TYPE_STRING;

				if(m_et == ElementType.PELEMENT_TYPE_VOID)
				{
					if(t == typeof(System.Object))
					{
						m_et = ElementType.PELEMENT_TYPE_OBJECT;
					}
					else if(t.IsEnum)
					{
						m_et = ElementType.PELEMENT_TYPE_VALUETYPE;
						m_flags |= c_Enum;
					}
					...
					{
						m_et = ElementType.PELEMENT_TYPE_CLASS;
						m_flags |= c_Interface;
					}
					else if(t.IsClass)
					{
			...
			internal void SetType(ElementType et)
			{
				...
				switch(m_et)
				{
					case ElementType.PELEMENT_TYPE_BOOLEAN:
						m_referenceType = typeof(System.Boolean);
						break;
					case ElementType.PELEMENT_TYPE_CHAR:
						m_referenceType = typeof(System.Char);
						break;
					case ElementType.PELEMENT_TYPE_I1:
						m_referenceType = typeof(System.SByte);
						break;
					case ElementType.PELEMENT_TYPE_U1:
						m_referenceType = typeof(System.Byte);
						break;
					case ElementType.PELEMENT_TYPE_I2:
						m_referenceType = typeof(System.Int16);
						break;
					case ElementType.PELEMENT_TYPE_U2:
						m_referenceType = typeof(System.UInt16);
						break;
					case ElementType.PELEMENT_TYPE_I4:
						m_referenceType = typeof(System.Int32);
						break;
					case ElementType.PELEMENT_TYPE_U4:
						m_referenceType = typeof(System.UInt32);
						break;
					case ElementType.PELEMENT_TYPE_I8:
						m_referenceType = typeof(System.Int64);
						break;
					case ElementType.PELEMENT_TYPE_U8:
						m_referenceType = typeof(System.UInt64);
						break;
					case ElementType.PELEMENT_TYPE_R4:
						m_referenceType = typeof(System.Single);
						break;
					case ElementType.PELEMENT_TYPE_R8:
						m_referenceType = typeof(System.Double);
						break;
					case ElementType.PELEMENT_TYPE_STRING:
						m_referenceType = typeof(System.String);
						break;

					default:
						throw Error(String.Format("Invalid primitive type: {0}", m_et));
				}
			}
		...
		class TypeHandler
		{
			...
			internal int EmitValue()
			{
				...
				if(m_type.m_base.IsPrimitive)
				{
					...
					{
						case ElementType.PELEMENT_TYPE_BOOLEAN:
							val = (bool)m_value ? 1UL : 0UL;
							break;
						case ElementType.PELEMENT_TYPE_CHAR:
							val = (ulong)(char)m_value;
							break;
						case ElementType.PELEMENT_TYPE_I1:
							val = (ulong)(sbyte)m_value;
							break;
						case ElementType.PELEMENT_TYPE_U1:
							val = (ulong)(byte)m_value;
							break;
						case ElementType.PELEMENT_TYPE_I2:
							val = (ulong)(short)m_value;
							break;
						case ElementType.PELEMENT_TYPE_U2:
							val = (ulong)(ushort)m_value;
							break;
						case ElementType.PELEMENT_TYPE_I4:
							val = (ulong)(int)m_value;
							break;
						case ElementType.PELEMENT_TYPE_U4:
							val = (ulong)(uint)m_value;
							break;
						case ElementType.PELEMENT_TYPE_I8:
							val = (ulong)(long)m_value;
							break;
						case ElementType.PELEMENT_TYPE_U8:
							val = (ulong)(ulong)m_value;
							break;
						case ElementType.PELEMENT_TYPE_R4:
							val = BytesFromFloat((float)m_value);
							break;
						case ElementType.PELEMENT_TYPE_R8:
							val = BytesFromDouble((double)m_value);
							break;
				...
				if(this.Hints_BitPacked != 0)
					bits = this.Hints_BitPacked;
				...
				if(fSigned)
				{
					...
					if(this.Hints_Scale != 0)
						valS /= (long)this.Hints_Scale;
				...
				else
				{
					switch(m_type.m_base.m_et)
					{
						...
						case ElementType.PELEMENT_TYPE_U8:
							...
							break;
						case ElementType.PELEMENT_TYPE_R4:
							m_value = FloatFromBytes(val);
							break;
						case ElementType.PELEMENT_TYPE_R8:
							m_value = DoubleFromBytes(val);
							break;
					}
				}
		...
		static public void LoadTypeFromAssembly(Assembly assm)
		{
			if(s_lstProcessedAssemblies.Contains(assm) == false)
			{
				...
				{
					if(s_fOnlySerializableObjects && t.IsSerializable == false)
						continue;
		...
		int SearchDuplicate(object o)
		{
			...
			if(m_htDuplicates.Contains(o) == false)
				return -1;
		...
		static internal void AddType(Type t)
		{
			...
			if(t.FullName.StartsWith("Microsoft.Internal.Deployment"))
				return;
			if(t.FullName == "ThisAssembly")
				return;
			if(t.FullName == "AssemblyRef")
				return;
		...
		internal uint ReadCompressedUnsigned()
		{
			...
			switch(val & 0xC0)
			{
				...
				case 0x40:
					break;
				case 0x80:
					val = ((val & ~0xC0U) << 8) | m_stream.ReadBits(8);
					break;
				case 0xC0:
					val = ((val & ~0xC0U) << 24) | m_stream.ReadBits(24);
					break;
			}
